...
exports.quickBuy =  async (req, res) => {
  try{
    const { type, fiat_currency_code, token_symbol, token_amount, fiat_amount } = req.query;
    if (!token_amount && !fiat_amount) {
      return successResponse(res, "No result found", []);
    }

    const token = await models.tokens.findOne({ where: { symbol: token_symbol } });
    const fiatCurrency = await models.fiat_currencies.findOne({ where: { code: fiat_currency_code } });
  ...
  if (token_amount && token_amount > 0) {
    whereConditions.total_available_amount = { [Op.lte]: token_amount };
  }

  if (fiat_amount && fiat_amount > 0) {
    whereConditions.price = { [Op.lte]: fiat_amount }; 
  }

  // Fetch the results
  console.log("whereConditions", whereConditions);
  let results = await models.lists.findAll({ where: whereConditions });
...
async function fetchedListLoop(element, banksIds = null) {
  ...
  //    console.log("element.dataValues.bank_id", element.dataValues.bank_id);
    ...
    if(element.fiat_currency_id !== "" || element.fiat_currency_id !== null){
      fiatCurrencyData  = await models.fiat_currencies.findByPk(element.fiat_currency_id);
    }
    let tokenData;
  ...
  //console.log("paymentMethodsIds", paymentMethods);
    if(element.payment_method_id !== "" || element.payment_method_id !== null){
